...
function zipBufferToTempFile(buf) {
  const tmpPath = path.join(DATA_DIR, "temp", `test-import-${Date.now()}-${Math.random().toString(36).slice(2)}.zip`);
...
describe("readProjectsData / writeProjectsData", () => {
  ...
  it("round-trips data through write then read", async () => {
    ...
    const testData = {
      ...
      activeProjectId: "abc-test",
    };
    await projectRepo.writeProjectsData(testData);
    const readBack = await projectRepo.readProjectsData();
...
describe("setActiveProject", () => {
  ...
  it("switches the active project", async () => {
    ...
    let data = await projectRepo.readProjectsData();
    ...
    data = await projectRepo.readProjectsData();
...
describe("updateProject", () => {
  ...
  it("preserves project files after folder rename", async () => {
    ...
    // Pages should exist
    const pagesDir = getProjectPagesDir(newFolderName);
